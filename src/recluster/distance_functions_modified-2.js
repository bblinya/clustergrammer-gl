...
console.log("=== distance function modified 2");
...
module.exports = {
  ...
  'cosine': function(vecA, vecB) {
    ...
    cos_dist = Math.max(cos_dist, 0.001);
  ...
  'correlation': function(vecA, vecB){
    ...
    const vecA_mag = vec_magnitude(vecA_diff_mean);
    const vecB_mag = vec_magnitude(vecB_diff_mean);
    if (vecA_mag == 0 && vecB_mag == 0) {
      return 0;
    } else if (vecA_mag == 0 || vecB_mag == 0) {
      return 1;
    } else {
      const cor_sim = vec_dot_product(
        vecA_diff_mean, vecB_diff_mean) / (vecA_mag * vecB_mag);
